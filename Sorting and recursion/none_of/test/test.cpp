...
TEST_CASE("Test 1: Niz od vise elemenata") {
  SUBCASE("Svi su parni") {
    ...
    INFO(t);
    REQUIRE(::none_of(t.begin(), t.end(), [](const auto& el) { return el % 2; }) == true);
  }

  SUBCASE("Jedan neparan") {
    std::vector<int> t{2, 4, 6, 8, 11};
    INFO(t);
    REQUIRE(::none_of(t.begin(), t.end(), [](const auto& el) { return el % 2; }) == false);
  }
...
TEST_CASE("Test 2: Niz od nula elemenata je tacan") {
  std::vector<int> t;
  CHECK(::none_of(t.begin(), t.end(), [](auto el) { return true; }));
}
